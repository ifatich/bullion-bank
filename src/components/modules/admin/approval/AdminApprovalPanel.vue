...
const showApprovalRequestDetail = (walletAddress: string) => {
  showAlert({
...
<template>
  <section class="approval-card" aria-labelledby="approval-title">
    ...
    <div class="approval-list">
      <div
        v-for="approvalRequest in approvalRequests"
        :key="approvalRequest.walletAddress + approvalRequest.approvalProgress"
        class="approval-row"
      >
        <span class="wallet-address">{{ approvalRequest.walletAddress }}</span>
        <span class="progress-badge">{{ approvalRequest.approvalProgress }}</span>
        <GButton
          class="detail-button"
          :icon="false"
          label="Detail"
          size="sm"
          type="outline-primary"
          @click="showApprovalRequestDetail(approvalRequest.walletAddress)"
        />
      </div>
    </div>
  </section>
